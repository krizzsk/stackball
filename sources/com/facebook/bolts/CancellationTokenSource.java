...
public final class CancellationTokenSource implements Closeable {
        ...
        if (!(j >= ((long) -1))) {
            ...
            synchronized (this.lock) {
                if (!this.cancellationRequested) {
                    cancelScheduledCancellation();
                    if (j != -1) {
                        this.scheduledCancellation = this.executor.schedule(new C2098xca2a191c(this, j, timeUnit), j, timeUnit);
                    }
                    Unit unit = Unit.INSTANCE;
                }
            }
    ...
    private final void notifyListeners(List<CancellationTokenRegistration> list) {
        for (CancellationTokenRegistration runAction$facebook_core_release : list) {
            runAction$facebook_core_release.runAction$facebook_core_release();
        }
    }
