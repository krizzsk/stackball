...
final class MlltSeeker implements Seeker {
    ...
    private MlltSeeker(long[] jArr, long[] jArr2, long j) {
        ...
        this.durationUs = j == -9223372036854775807L ? C3716C.msToUs(jArr2[jArr2.length - 1]) : j;
    }

    public SeekMap.SeekPoints getSeekPoints(long j) {
        Pair<Long, Long> linearlyInterpolate = linearlyInterpolate(C3716C.usToMs(Util.constrainValue(j, 0, this.durationUs)), this.referenceTimesMs, this.referencePositions);
        return new SeekMap.SeekPoints(new SeekPoint(C3716C.msToUs(((Long) linearlyInterpolate.first).longValue()), ((Long) linearlyInterpolate.second).longValue()));
    }

    public long getTimeUs(long j) {
        return C3716C.msToUs(((Long) linearlyInterpolate(j, this.referencePositions, this.referenceTimesMs).second).longValue());
    }
