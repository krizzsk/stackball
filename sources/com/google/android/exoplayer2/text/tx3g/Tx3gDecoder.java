...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    public Tx3gDecoder(List<byte[]> list) {
        ...
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultFontFamily = TX3G_SERIF.equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? "serif" : str;
            this.calculatedVideoTrackHeight = bArr[25] * Ascii.DC4;
            z = (bArr[0] & 32) == 0 ? false : z;
            this.customVerticalPlacement = z;
            if (z) {
                this.defaultVerticalPlacement = Util.constrainValue(((float) ((bArr[11] & 255) | ((bArr[10] & 255) << 8))) / ((float) this.calculatedVideoTrackHeight), 0.0f, 0.95f);
            } else {
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray2, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.m9247w(TAG, "Truncating styl end (" + readUnsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            i = spannableStringBuilder.length();
        } else {
            i = readUnsignedShort2;
        }
        if (readUnsignedShort >= i) {
            Log.m9247w(TAG, "Ignoring styl with start (" + readUnsignedShort + ") >= end (" + i + ").");
            return;
        }
        SpannableStringBuilder spannableStringBuilder2 = spannableStringBuilder;
