...
public final class Cea708Decoder extends CeaDecoder {
        ...
        while (this.ccData.bytesLeft() >= 3) {
            ...
            if ((i == 2 || i == 3) && z2) {
                if (i == 3) {
                    ...
                    if (!(i3 == -1 || i2 == (i3 + 1) % 4)) {
                        resetCueBuilders();
                        Log.m9247w(TAG, "Sequence number discontinuity. previous=" + this.previousSequenceNumber + " current=" + i2);
                    }
                    ...
                    if (dtvCcPacket3 == null) {
                        Log.m9243e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                    } else {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.m9241d(TAG, "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + ");");
        }
        ...
        if (readBits == 7) {
            ...
            if (readBits < 7) {
                Log.m9247w(TAG, "Invalid extended service number: " + readBits);
            }
        }
        if (readBits2 == 0) {
            if (readBits != 0) {
                Log.m9247w(TAG, "serviceNumber is non-zero (" + readBits + ") when blockSize is 0");
            }
            ...
            while (this.serviceBlockPacket.bitsLeft() > 0) {
                ...
                if (readBits3 == 16) {
                    ...
                    if (readBits4 <= 31) {
                        ...
                        Log.m9247w(TAG, "Invalid extended command: " + readBits4);
                    }
                    ...
                    Log.m9247w(TAG, "Invalid base command: " + readBits3);
                }
                z = true;
            }
            if (z) {
    ...
    private void handleC0Command(int i) {
        ...
        if (i == 3) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m9247w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.serviceBlockPacket.skipBits(8);
                        return;
                    } else if (i < 24 || i > 31) {
                        Log.m9247w(TAG, "Invalid C0 command: " + i);
                        return;
                    } else {
                        Log.m9247w(TAG, "Currently unsupported COMMAND_P16 Command: " + i);
    ...
    private void handleG2Character(int i) {
        if (i == 32) {
            ...
            this.currentCueInfoBuilder.append(Typography.f44214tm);
        } else if (i == 58) {
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            return;
                        default:
                            Log.m9247w(TAG, "Invalid G2 character: " + i);
                            return;
                    }
            }
        } else {
    ...
    private void handleG3Character(int i) {
        ...
        Log.m9247w(TAG, "Invalid G3 character: " + i);
    ...
    private static final class Cea708CueInfo {
        ...
        public static final Comparator<Cea708CueInfo> LEAST_IMPORTANT_FIRST = C3862xde5f1e35.INSTANCE;
