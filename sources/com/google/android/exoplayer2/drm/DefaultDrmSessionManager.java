...
public class DefaultDrmSessionManager implements DrmSessionManager {
    ...
    public static final class Builder {
        ...
        private UUID uuid = C3716C.WIDEVINE_UUID;
    ...
    private DrmSession maybeAcquirePlaceholderSession(int i) {
        ...
        if (defaultDrmSession == null) {
            DefaultDrmSession createAndAcquireSessionWithRetry = createAndAcquireSessionWithRetry(ImmutableList.m9346of(), true, (DrmSessionEventListener.EventDispatcher) null);
    ...
    private boolean canAcquireSession(DrmInitData drmInitData) {
        ...
        if (getSchemeDatas(drmInitData, this.uuid, true).isEmpty()) {
            if (drmInitData.schemeDataCount != 1 || !drmInitData.get(0).matches(C3716C.COMMON_PSSH_UUID)) {
                return false;
            }
            Log.m9247w(TAG, "DrmInitData only contains common PSSH SchemeData. Assuming support for: " + this.uuid);
        }
    ...
    private static List<DrmInitData.SchemeData> getSchemeDatas(DrmInitData drmInitData, UUID uuid2, boolean z) {
        ...
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if ((schemeData.matches(uuid2) || (C3716C.CLEARKEY_UUID.equals(uuid2) && schemeData.matches(C3716C.COMMON_PSSH_UUID))) && (schemeData.data != null || z)) {
