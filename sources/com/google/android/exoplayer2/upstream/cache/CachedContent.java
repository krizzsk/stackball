...
final class CachedContent {
    ...
    public SimpleCacheSpan setLastTouchTimestamp(SimpleCacheSpan simpleCacheSpan, long j, boolean z) {
        ...
        if (z) {
            File cacheFile = SimpleCacheSpan.getCacheFile((File) Assertions.checkNotNull(file.getParentFile()), this.f10071id, simpleCacheSpan.position, j);
            if (file.renameTo(cacheFile)) {
                file = cacheFile;
            } else {
                Log.m9247w(TAG, "Failed to rename " + file + " to " + cacheFile);
            }
        }
    ...
    public int hashCode() {
        return (((this.f10071id * 31) + this.key.hashCode()) * 31) + this.metadata.hashCode();
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.f10071id != cachedContent.f10071id || !this.key.equals(cachedContent.key) || !this.cachedSpans.equals(cachedContent.cachedSpans) || !this.metadata.equals(cachedContent.metadata)) {
            return false;
        }
        return true;
    }
