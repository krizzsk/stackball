...
public class BidTokenEncoder {
    ...
    static int getAvailableSizeForHBT(int i, String str, String str2) {
        return (int) Math.max(Math.round(((double) ((int) (((Math.floor((double) (((i - str.getBytes().length) - CertificateUtil.DELIMITER.getBytes().length) / 4)) * 3.0d) - ((double) CertificateUtil.DELIMITER.getBytes().length)) - ((double) str2.getBytes().length)))) / 4.0d) * 4, 0);
    }
}
