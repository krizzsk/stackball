package com.chartboost.sdk.Banner;

import com.smaato.sdk.video.vast.model.ErrorCode;

public enum BannerSize {
    STANDARD,
    MEDIUM,
    LEADERBOARD;
    

    /* renamed from: a */
    private static final C1818f f4337a = null;

    /* renamed from: b */
    private static final C1818f f4338b = null;

    /* renamed from: c */
    private static final C1818f f4339c = null;

    /* renamed from: com.chartboost.sdk.Banner.BannerSize$a */
    static /* synthetic */ class C1811a {

        /* renamed from: a */
        static final /* synthetic */ int[] f4341a = null;

        /* JADX WARNING: Can't wrap try/catch for region: R(6:0|1|2|3|4|(3:5|6|8)) */
        /* JADX WARNING: Failed to process nested try/catch */
        /* JADX WARNING: Missing exception handler attribute for start block: B:3:0x0012 */
        /* JADX WARNING: Missing exception handler attribute for start block: B:5:0x001d */
        static {
            /*
                com.chartboost.sdk.Banner.BannerSize[] r0 = com.chartboost.sdk.Banner.BannerSize.values()
                int r0 = r0.length
                int[] r0 = new int[r0]
                f4341a = r0
                com.chartboost.sdk.Banner.BannerSize r1 = com.chartboost.sdk.Banner.BannerSize.STANDARD     // Catch:{ NoSuchFieldError -> 0x0012 }
                int r1 = r1.ordinal()     // Catch:{ NoSuchFieldError -> 0x0012 }
                r2 = 1
                r0[r1] = r2     // Catch:{ NoSuchFieldError -> 0x0012 }
            L_0x0012:
                int[] r0 = f4341a     // Catch:{ NoSuchFieldError -> 0x001d }
                com.chartboost.sdk.Banner.BannerSize r1 = com.chartboost.sdk.Banner.BannerSize.MEDIUM     // Catch:{ NoSuchFieldError -> 0x001d }
                int r1 = r1.ordinal()     // Catch:{ NoSuchFieldError -> 0x001d }
                r2 = 2
                r0[r1] = r2     // Catch:{ NoSuchFieldError -> 0x001d }
            L_0x001d:
                int[] r0 = f4341a     // Catch:{ NoSuchFieldError -> 0x0028 }
                com.chartboost.sdk.Banner.BannerSize r1 = com.chartboost.sdk.Banner.BannerSize.LEADERBOARD     // Catch:{ NoSuchFieldError -> 0x0028 }
                int r1 = r1.ordinal()     // Catch:{ NoSuchFieldError -> 0x0028 }
                r2 = 3
                r0[r1] = r2     // Catch:{ NoSuchFieldError -> 0x0028 }
            L_0x0028:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.chartboost.sdk.Banner.BannerSize.C1811a.<clinit>():void");
        }
    }

    static {
        f4337a = new C1818f(320, 50);
        f4338b = new C1818f(ErrorCode.GENERAL_WRAPPER_ERROR, 250);
        f4339c = new C1818f(728, 90);
    }

    /* renamed from: a */
    private static C1818f m3929a(BannerSize bannerSize) {
        int i = C1811a.f4341a[bannerSize.ordinal()];
        if (i == 1) {
            return f4337a;
        }
        if (i == 2) {
            return f4338b;
        }
        if (i != 3) {
            return null;
        }
        return f4339c;
    }

    public static BannerSize fromInteger(int i) {
        if (i == 0) {
            return STANDARD;
        }
        if (i == 1) {
            return MEDIUM;
        }
        if (i != 2) {
            return null;
        }
        return LEADERBOARD;
    }

    public static int getHeight(BannerSize bannerSize) {
        C1818f a = m3929a(bannerSize);
        if (a != null) {
            return a.mo14174a();
        }
        return 0;
    }

    public static int getWidth(BannerSize bannerSize) {
        C1818f a = m3929a(bannerSize);
        if (a != null) {
            return a.mo14175b();
        }
        return 0;
    }
}
